...
sep_old <- 
  ...
  group_by(id, name, title, department) %>%
  summarize(u_grad = sum(u_grad), .groups = "drop") %>%
  filter(u_grad > 10)
sep_old

# Added group_by and summarize to compress Gov 50 from two rows to one
```
...
# Being mindful of the data (decreased rows by 7 meaning 7 classes/sections 
# dropped)

# focus our attention on undergrad enrollment instead of grad enrollment or
# other people

# global pandemic...people taking leaves of absences, etcc


After we have a model, Temperance suggests we should:
# not assume reasons for why enrollment dropped / why individual students made 
...
```{r}
sep_diff <- 
  inner_join(sep_new, sep_old, by = c("id", "name", "title", "department")) %>%
